import React from "react";
import DashboardLayout from "@/components/dashboard/DashboardLayout";
import { Skeleton, Card } from "@/ui";

export const ResumesSkeleton: React.FC = () => {
  return (
    <DashboardLayout>
      {/* Header Skeleton */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
        <div className="space-y-2">
          <Skeleton className="h-8 md:h-9 w-52 md:w-64 rounded-xl" />
          <Skeleton className="h-4 w-72 sm:w-96 rounded-md bg-muted/60" />
        </div>
        <Skeleton className="h-10 w-40 sm:w-44 rounded-xl self-start sm:self-auto shrink-0 bg-sky-500/20" />
      </div>

      {/* Resumes Grid Skeleton */}
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {Array.from({ length: 6 }).map((_, index) => (
          <Card
            key={index}
            variant="glass"
            className="p-5 flex flex-col justify-between border border-border/80 shadow-sm"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <Skeleton className="w-10 h-10 rounded-xl bg-sky-500/15" />
                <div className="flex items-center gap-1.5">
                  <Skeleton className="w-7 h-7 rounded-lg bg-muted/60" />
                </div>
              </div>
              <Skeleton className="h-5 w-3/4 rounded-md mb-3 bg-muted/90" />
              <div className="flex items-center gap-2">
                <Skeleton className="w-3.5 h-3.5 rounded bg-muted/60" />
                <Skeleton className="h-3 w-32 rounded bg-muted/60" />
              </div>
            </div>
          </Card>
        ))}
      </div>
    </DashboardLayout>
  );
};

export default ResumesSkeleton;
